...
//空白MAC地址
static UCHAR g_EmptyMacAddress[6] = {0,0,0,0,0,0};
//广播地址
static UCHAR g_BroadcastMacAddress[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
//!添加代码!
...
BOOLEAN IfReturnNBL(IN PLCXL_FILTER pFilter, IN PNDIS_ETH_HEADER pEthHeader, IN UINT BufferLength)
{
    ...
    case ETHERNET_IPV6://IPv6协议
		break;
    case ETHERNET_ARP://ARP协议
        {
            PARP_PACKET pArpPacket;

            pArpPacket = (PARP_PACKET)((PUCHAR)pEthType+sizeof(USHORT));
            KdPrint(("recv ARP packet\n"));
            switch(pArpPacket->OperateCode) {
            case ARP_QUERY://ARP查询
