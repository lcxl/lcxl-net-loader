...
--*/
{
    ...
    do
    {
		...
		//创建退出事件
		KeInitializeEvent(&g_setting.thread_exit_event, NotificationEvent, FALSE);
		InitializeObjectAttributes(&oa, NULL, OBJ_KERNEL_HANDLE, NULL, NULL);
		KeQueryPerformanceCounter(&g_setting.frequency);
		//创建系统线程
		Status = PsCreateSystemThread(&g_setting.thread_handle, GENERIC_ALL, &oa, NULL, NULL, FilterWorkThread, (PVOID)&g_setting.thread_exit_event);
		if (NT_SUCCESS(Status)) {
			//获取线程句柄
			Status = ObReferenceObjectByHandle(g_setting.thread_handle,
				0,
				*PsThreadType,
				KernelMode,
				(PVOID *)&g_setting.thread_object,
				NULL
				);
		}
...
--*/
{
    ...
    do
    {
        ...
        filter = (PLCXL_FILTER)FILTER_ALLOC_MEM(NdisFilterHandle, Size);
        if (filter == NULL)
        {
        ...
        NdisZeroMemory(filter, sizeof(LCXL_FILTER));
		//初始化设置
		InitModuleSetting(&filter->module, AttachParameters);
		//初始化kalman滤波数据
		InitKalmanState(&filter->routing_algorithm_data.ra_fast_respnse.kalman_state);
		//初始化服务器列表
		//InitializeListHead(&pFilter->server_list.list_entry);
		//初始化路由列表
		InitializeListHead(&filter->route_list);
        //初始化NBL发送池
        ...
        filter->send_net_buffer_list_pool = NdisAllocateNetBufferListPool( NdisFilterHandle, &PoolParameters); 
        ...
        //
        filter->track_receives = TRUE;
        filter->track_sends = TRUE;
        filter->filter_handle = NdisFilterHandle;
...
--*/
{
	...
	//激活退出事件
	KeSetEvent(&g_setting.thread_exit_event, 0, TRUE);
	//等待线程退出
	KeWaitForSingleObject(g_setting.thread_object,
		Executive,
		KernelMode,
		FALSE,
		NULL);
	...
	//减少引用
	ObDereferenceObject(g_setting.thread_object);
	//关闭线程句柄
	ZwClose(g_setting.thread_handle);
    //
...
--*/
{
	...
	while (current_nbl)
	{
		...
		case PNRC_PASS:case PNRC_MODIFY:
			...
			break;
		case PNRC_DROP:case PNRC_ICMPV6_NS:
			//拦截此NBL
			...
			if (return_data.code == PNRC_ICMPV6_NS) {
	...
	while (current_nbl != NULL) {
		...
		case PNRC_DROP:case PNRC_ROUTER:case PNRC_ICMPV6_NA:
			...
			case PNRC_ROUTER://如果需要路由此数据包
			{
				...
				break;
			case PNRC_ICMPV6_NA://如果需要修改此NA数据包
			{
		...
		case PNRC_PASS:
			...
			if (return_data.code == PNRC_CHECKING_NBL) {
				...
				LockFilter(filter, &lock_handle);
				checking_header.preformance = filter->module.performance;
				UnlockFilter(&lock_handle);
				
				//发送响应包
				CreateCheckingNBL(filter, &ethernet_header->Source, &checking_header, &send_nbl);
				//插入到转发队列中
				if (send_nbl_head == NULL) {
					send_nbl_head = send_nbl;
					send_nbl_tail = send_nbl;
...
PLCXL_ROUTE_LIST_ENTRY RouteTCPNBL(IN PLCXL_FILTER filter, IN INT ip_mode, IN PVOID ip_header) 
{
	...
	switch (ip_mode) {
	case IM_IPV4:
		ptcp_header = (PTCP_HDR)((PUCHAR)ip_header + Ip4HeaderLengthInBytes((PIPV4_HEADER)ip_header));
		break;
	case IM_IPV6:
		ptcp_header = (PTCP_HDR)((PUCHAR)ip_header + sizeof(IPV6_HEADER));
		break;
	default:
		ASSERT(FALSE);
		break;
	}
	route_info = GetRouteListEntry(&filter->route_list, filter->module.route_timeout, &filter->module.server_list, ip_mode, ip_header, ptcp_header);
	//建立连接的阶段
	...
	if (ptcp_header->th_flags == TH_SYN) {
		...
		//初始化路由信息
		InitRouteListEntry(route_info, ip_mode, ip_header, ptcp_header, server);
		KdPrint((
...
VOID CheckServerStatus(IN PLCXL_FILTER filter)
{
	...
	while (Link != head) {
		...
		if (send_check_nbl) {
			...
			LCXL_CHECKING_HEADER checking_header = { 0 };

			checking_header.code = CHECKING_CODE_REQUEST;
			
			//发送数据包
			CreateCheckingNBL(filter, (PDL_EUI48)server_info->info.mac_addr.Address, &checking_header, &send_nbl);
			//插入到转发队列中
			if (send_nbl_head == NULL) {
				send_nbl_head = send_nbl;
				send_nbl_tail = send_nbl;
...
{
	if (is_recv) {
		if (RtlCompareMemory(eth_header->Destination.Byte, mac_addr->Address, sizeof(eth_header->Destination.Byte)) < sizeof(eth_header->Destination.Byte) &&
			RtlCompareMemory(eth_header->Destination.Byte, AnycastMacAddr.Byte, sizeof(eth_header->Destination.Byte)) < sizeof(eth_header->Destination.Byte)){
			KdPrint((
				"SYS:CheckTCPNBLMacAddr recv pEthHeader->Destination(%02x:%02x:%02x:%02x:%02x:%02x) != filter->module.mac_addr.Address\n",
				eth_header->Destination.Byte[0],
				eth_header->Destination.Byte[1],
				eth_header->Destination.Byte[2],
				eth_header->Destination.Byte[3],
				eth_header->Destination.Byte[4],
				eth_header->Destination.Byte[5]));
			return FALSE;
		}
	} else {
		if (RtlCompareMemory(eth_header->Source.Byte, mac_addr->Address, sizeof(eth_header->Source.Byte)) < sizeof(eth_header->Source.Byte) &&
			RtlCompareMemory(eth_header->Source.Byte, AnycastMacAddr.Byte, sizeof(eth_header->Source.Byte)) < sizeof(eth_header->Source.Byte)) {
			KdPrint((
				"SYS:CheckTCPNBLMacAddr send pEthHeader->Source(%02x:%02x:%02x:%02x:%02x:%02x) != filter->module.mac_addr.Address\n",
				eth_header->Source.Byte[0],
				eth_header->Source.Byte[1],
				eth_header->Source.Byte[2],
				eth_header->Source.Byte[3],
				eth_header->Source.Byte[4],
				eth_header->Source.Byte[5]));
			return FALSE;
		}
	}
	return TRUE;
...
{
	//
	PKEVENT exit_event = (PKEVENT)StartContext;
	...
	while (status = KeWaitForSingleObject(exit_event, Executive, KernelMode, FALSE, &timeout), NT_SUCCESS(status)) {
		...
		if (status == STATUS_TIMEOUT) {
			...
			while (list_entry != list_head) {
				//KLOCK_QUEUE_HANDLE lock_handle;
				PLCXL_FILTER filter = GetFilterByListEntry(list_entry);
				//LockFilter(filter, &lock_handle);
	...
	KdPrint(("SYS:FilterWorkThread ended.\n"));
...
VOID ProcessNBL(IN PLCXL_FILTER filter, IN BOOLEAN is_recv, IN PETHERNET_HEADER eth_header, IN UINT data_length, IN OUT PPROCESS_NBL_RESULT return_data)
{
	...
	case ETHERNET_TYPE_LCXL_CHECKING:
		//处理可用性检测帧
		if (data_length >= sizeof(LCXL_CHECKING_HEADER)) {
			ProcessCheckingNBL(filter, is_recv, (PETHERNET_LCXL_CHECKING)eth_header, return_data);
		}
		break;
...
VOID ProcessCheckingNBL(IN PLCXL_FILTER filter, IN BOOLEAN is_recv, IN PETHERNET_LCXL_CHECKING eth_header, IN OUT PPROCESS_NBL_RESULT return_data)
{
	...
	if (is_recv) {
		...
		if (eth_header->checking_header.code == CHECKING_CODE_REQUEST) {
		...
		case LCXL_ROLE_ROUTER:
		{
			...
			if (server_info != NULL) {
				...
				//取消检查状态
				server_info->info.status &= ~SS_CHECKING;
				if (eth_header->checking_header.code == CHECKING_CODE_RESPONSE) {
					//更新性能
					server_info->performance = eth_header->checking_header.preformance;
				}
				UnLockServer(&lock_handle);
			}
			//解锁列表
...
VOID ProcessICMPv6(IN PLCXL_FILTER filter, IN BOOLEAN is_recv, IN PIPV6_HEADER ip_header, IN PLCXL_ADDR_INFO virtual_addr, IN OUT PPROCESS_NBL_RESULT return_data)
{
	...
	case LCXL_ROLE_SERVER:
		...
		case 133://路由器请求（Router Solicitation，RS）
		...
		case 135://邻居请求(Neighbor Solicitation，NS)
			...
			if (!is_recv) {
				...
				if (RtlCompareMemory(&ip_header->DestinationAddress, &solicited_node_multicast_address, sizeof(ip_header->DestinationAddress)) == sizeof(ip_header->DestinationAddress) ||
					...
					if (RtlCompareMemory(&ip_header->SourceAddress, &virtual_addr->ipv6, sizeof(ip_header->SourceAddress)) == sizeof(ip_header->SourceAddress)) {
						return_data->code = PNRC_ICMPV6_NS;
			...
			break;
		case 136://邻居宣告（Neighbor Advertisement，NA）
			//如果发现有别的主机对虚拟IP进行宣告
			if (RtlCompareMemory(&ip_header->SourceAddress, &virtual_addr->ipv6, sizeof(ip_header->SourceAddress)) == sizeof(ip_header->SourceAddress)) {
				if (is_recv) {
				...
				if (RtlCompareMemory(&ip_header->DestinationAddress, &virtual_addr->ipv6, sizeof(ip_header->DestinationAddress)) == sizeof(ip_header->DestinationAddress)) {
					if (is_recv) {
						...
						return_data->code = PNRC_ICMPV6_NA;
	...
	case LCXL_ROLE_ROUTER://如果角色是路由
		...
		if (is_recv) {
			...
			if (RtlCompareMemory(destination_address, virtual_ip, destination_address_len) == destination_address_len) {
				// 创建路由表
				return_data->data.route = RouteTCPNBL(filter, ip_mode, ip_header);
				// 如果路由表不为空
	...
	case LCXL_ROLE_SERVER:
		if (!is_recv) {
			...
			if (RtlCompareMemory(source_address, virtual_ip, source_address_len) == source_address_len) {
				...
				return_data->data.modifyip.ip_mode = ip_mode;
...
{
	PETHERNET_LCXL_CHECKING send_buffer;
	UINT send_buffer_length;
	//以太网帧最小长度
	send_buffer_length = sizeof(ETHERNET_LCXL_CHECKING)>64 ? sizeof(ETHERNET_LCXL_CHECKING): 64;
	send_buffer = (PETHERNET_LCXL_CHECKING)FILTER_ALLOC_MEM(filter->filter_handle, send_buffer_length);
	RtlZeroMemory(send_buffer, send_buffer_length);
	//拷贝mac地址
	NdisMoveMemory(&send_buffer->eth_header.Source, filter->module.mac_addr.Address, sizeof(send_buffer->eth_header.Source));
	send_buffer->eth_header.Destination = *dest_mac;
	send_buffer->eth_header.Type = ntohs(ETHERNET_TYPE_LCXL_CHECKING);
	send_buffer->checking_header = *checking_header;
	return CreateNBL(filter, &send_buffer->eth_header, send_buffer_length, nbl);
}
